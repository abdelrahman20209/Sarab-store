...
import { 
    getFirestore, collection, addDoc, getDocs, doc, deleteDoc, updateDoc, serverTimestamp, getDoc 
...
const IMGBB_API_KEY = "96af28066f391086bdad4447630083a2"; // ⚠️ ضع مفتاح API الخاص بك هنا
...
const updatePricePreview = () => {
    ...
    } else { if(pricePreview) pricePreview.innerText = ""; }
};

hasDiscountSelect?.addEventListener("change", () => {
    discountSection.classList.toggle("hidden-fade", hasDiscountSelect.value === "no");
    updatePricePreview();
});

[prodPriceInput, prodDiscountInput].forEach(input => input?.addEventListener("input", updatePricePreview));
...
productForm?.addEventListener("submit", async (e) => {
    ...
    btn.disabled = true;
    btn.innerText = "جاري الحفظ في الأسطول... ⏳";
    ...
    try {
        ...
        // إذا كان هناك رابط صورة جديد أضفه للبيانات
        if (imageUrl) productData.image = imageUrl;

        if (currentEditId) {
            await updateDoc(doc(db, "products", currentEditId), productData);
            statusMessage.innerHTML = "<span style='color:#00ff00'>تم تحديث المنتج بنجاح! ✅</span>";
        } else {
            // في حالة الإضافة الجديدة يجب وجود صورة
            if (!imageUrl) {
                alert("برجاء اختيار صورة للمنتج الجديد!");
                btn.disabled = false;
                btn.innerText = "إضافة المنتج للأسطول 🚀";
                return;
            }
            productData.createdAt = serverTimestamp();
            await addDoc(collection(db, "products"), productData);
            statusMessage.innerHTML = "<span style='color:#00ff00'>تمت الإضافة للأسطول! 🚀</span>";
        }
        ...
        console.error(error);
        statusMessage.innerHTML = "<span style='color:red'>خطأ: " + error.message + "</span>";
    } finally { 
        btn.disabled = false; 
    }
});
...
async function loadProducts() {
    ...
    adminProductsList.innerHTML = "<p>جاري تحميل الأسطول...</p>";
    try {
        ...
        querySnapshot.forEach((docSnap) => {
            ...
            card.style = "background:#151515; padding:15px; margin-bottom:10px; border-radius:8px; display:flex; justify-content:space-between; align-items:center; border: 1px solid #333;";
            card.innerHTML = `
...
// --- وظائف الحذف والتعديل العالمية ---
window.deleteProduct = async (id) => {
    if (confirm("هل أنت متأكد من حذف هذا العطر من الأسطول؟")) { 
        try {
...
window.editProduct = async (id) => {
    ...
    } catch (err) { alert("فشل تحميل البيانات: " + err.message); }
};
